...
// 求解k-core的主流程
void KCore(const EdgeT *cudaNodeIndex, const NodeT *cudaAdj, NodeT nodesNum, NodeT *&cudaCore) {
  auto *nodeIndex = (EdgeT *)MyMalloc((nodesNum + 1) * sizeof(EdgeT));
  ...
  auto *adj = (NodeT *)MyMalloc(edgesNum * sizeof(NodeT));
  CUDA_TRY(cudaMemcpy(adj, cudaAdj, edgesNum * sizeof(NodeT), cudaMemcpyDeviceToHost));

  auto *core = (NodeT *)MyMalloc(nodesNum * sizeof(NodeT));
...
#pragma omp barrier
  ...
  CUDA_TRY(cudaMallocManaged((void **)&cudaCore, nodesNum * sizeof(NodeT)));
  CUDA_TRY(cudaMemcpy(cudaCore, core, nodesNum * sizeof(NodeT), cudaMemcpyHostToDevice));

  MyFree((void *&)curr, nodesNum * sizeof(NodeT));
  MyFree((void *&)next, nodesNum * sizeof(NodeT));
  MyFree((void *&)nodeIndex, (nodesNum + 1) * sizeof(EdgeT));
  MyFree((void *&)adj, edgesNum * sizeof(NodeT));
  MyFree((void *&)core, nodesNum * sizeof(NodeT));
}
