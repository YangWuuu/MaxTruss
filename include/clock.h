...
class Clock {
 public:
  explicit Clock(const std::string &str_) {
    s_ = str_;
  }

  const char *Start() {
    startTime_ = duration_cast<microseconds>(high_resolution_clock::now().time_since_epoch()).count();
    sprintf(str_, "[\033[1m\033[44;30m%-12s\033[0m] Start...", s_.c_str());
    return str_;
  }
  ...
  const char *Count(const char *fmt = "", ...) {
    ...
    sprintf(str_, "[\033[1m\033[44;31m%-12s\033[0m] %.6lfs   %s", s_.c_str(), t, str2);
    return str_;
  }
